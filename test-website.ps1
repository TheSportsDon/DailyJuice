...
foreach ($image in $imageFiles) {
    ...
    if (Test-Path $path) {
        ...
        Write-Host "  [OK] $image ($([math]::Round($size, 2)) KB)" -ForegroundColor Green
        $passes++
        
        # Warn if file is too large
        if ($size -gt 500) {
            Write-Host "    [WARN] File size > 500KB" -ForegroundColor Yellow
            $warnings++
        }
    } else {
...
foreach ($image in $largeImages) {
    ...
    if (Test-Path $path) {
        ...
        Write-Host "  [LARGE] $image is $([math]::Round($size, 2)) MB (too large!)" -ForegroundColor Yellow
        $warnings++
    } else {
        Write-Host "  [OK] $image not found (OK if using WebP fallback)" -ForegroundColor Green
        $passes++
    }
}
